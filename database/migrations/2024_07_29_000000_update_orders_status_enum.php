...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if ($driver === 'sqlite') {
            ...
            // 复制数据
            DB::statement("
                INSERT INTO orders_new (id, user_id, total_amount, status, created_at, updated_at)
            ...
            // 如果processing不在当前枚举值中，添加它
            if (!in_array('processing', $enumValues)) {
                // 临时修改为VARCHAR
                DB::statement("ALTER TABLE orders MODIFY COLUMN status VARCHAR(20) DEFAULT 'pending'");
                
                // 更新任何无效的状态值
                DB::statement("UPDATE orders SET status = 'pending' WHERE status NOT IN ('pending', 'processing', 'completed', 'cancelled')");
    ...
    public function down(): void
    {
        ...
        if ($driver === 'sqlite') {
            ...
            // 复制数据，将processing状态改为pending
            DB::statement("
                INSERT INTO orders_old (id, user_id, total_amount, status, created_at, updated_at)
                SELECT id, user_id, total_amount, 
                       CASE 
